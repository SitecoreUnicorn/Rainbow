...
{
	public class ComparerReplayDeserializer : IDeserializer
	{
		private readonly IDefaultDeserializerLogger _logger;
		private readonly IItemComparer _comparer;

		public ComparerReplayDeserializer(IDefaultDeserializerLogger logger, IItemComparer comparer)
		{
			Assert.ArgumentNotNull(logger, "logger");
			Assert.ArgumentNotNull(comparer, "comparer");
			// TODO: optimize writes to item with delegate system and one editcontext?
			// TODO: implement compare replayer
			_logger = logger;
			_comparer = comparer;
		}

		public ISerializableItem Deserialize(ItemComparisonResult comparisonResult, bool ignoreMissingTemplateFields)
		{
			Assert.ArgumentNotNull(comparisonResult, "comparisonResult");
			Assert.IsNotNull(comparisonResult.SourceItem, "Source item cannot be null when writing a comparison result.");

			// note: we assume the "source" side of the comparison is our source, and the target side is what we're writing to
			Database database = Factory.GetDatabase(comparisonResult.SourceItem.DatabaseName);

			Item destinationParentItem = database.GetItem(new ID(comparisonResult.SourceItem.ParentId));
			Item targetItem = database.GetItem(new ID(comparisonResult.SourceItem.Id));
			bool newItemWasCreated = false;

			// the target item did not yet exist, so we need to start by creating it
			if (targetItem == null)
			{
				targetItem = CreateTargetItem(comparisonResult.SourceItem, destinationParentItem);
			...
			else
			{
				...
				if (destinationParentItem == null)
				{
					...
					{
						ParentID = new ID(comparisonResult.SourceItem.ParentId).ToString(),
						Item = targetItem
					};
				}
			...
			try
			{
				ChangeTemplateIfNeeded(comparisonResult.SourceItem, targetItem);
				RenameIfNeeded(comparisonResult.SourceItem, targetItem);
				...
				using (new EditContext(targetItem))
				{
					...
					{
						if (field.Shared && comparisonResult.SourceItem.SharedFields.All(x => x.FieldId != field.ID.Guid))
						{
					...
					foreach (var field in comparisonResult.SourceItem.SharedFields)
						PasteSyncField(targetItem, field, ignoreMissingTemplateFields, newItemWasCreated);
				}

				ClearCaches(database, new ID(comparisonResult.SourceItem.Id));
				targetItem.Reload();
				ResetTemplateEngineIfItemIsTemplate(targetItem);
				...
				foreach (var syncVersion in comparisonResult.SourceItem.Versions)
				{
			...
			catch (Exception ex)
			{
				...
				throw new Exception("Failed to paste item: " + comparisonResult.SourceItem.Path, ex);
			}
		}
		...
		{
			if (!_comparer.Includes(field.FieldId))
			{
